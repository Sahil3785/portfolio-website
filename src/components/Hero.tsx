...
const Hero = () => {
  ...
  const downloadCV = () => {
    // Open PDF in new tab for browser viewing
    window.open('/Images/Sahil-Resume.pdf', '_blank');
  };
  ...
  return (
    <section id="hero" ref={heroRef} className="relative min-h-screen flex items-center justify-center overflow-hidden">
      <div ref={splineRef} className="absolute inset-0 w-full h-full opacity-70 overflow-hidden">
        ...
        />
        {/* Hide Spline watermark in bottom-right corner */}
        <div 
          className="absolute bottom-0 right-0 w-40 h-16 bg-background z-10"
          style={{ 
            background: 'linear-gradient(135deg, transparent 0%, hsl(var(--background)) 30%)',
            pointerEvents: 'none'
          }} 
        />
      </div>
      ...
      <div className="relative z-10 text-center px-6 max-w-4xl mx-auto">
        <h1 ref={titleRef} className="text-4xl md:text-6xl lg:text-7xl font-light text-foreground mb-6 leading-tight">
          ...
          <span className="text-primary-glow">Frontend & Automation Developer</span>
        </h1>

        <p ref={subtitleRef} className="text-lg md:text-xl text-muted-foreground mb-8 max-w-2xl mx-auto leading-relaxed">
          Automation-focused Software Developer building API-driven systems, automation workflows, and scalable integrations with React, TypeScript, and modern backend platforms
        </p>
